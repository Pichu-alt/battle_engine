...
void update_cursor_action_select(u8 t_id)
{
    switch (gMain.newKeys & (KEY_DPAD | KEY_A)) {
        ...
        case KEY_A:
            ...
            obj_free(&gSprites[battle_master->battle_cursor.objid]);
            bs_anim_status = 0;
            break;
        default:
    ...
    gSprites[battle_master->battle_cursor.objid].pos1.x = battle_master->battle_cursor.cursor_x[battle_master->battle_cursor.cursor_pos];
    gSprites[battle_master->battle_cursor.objid].pos1.y = battle_master->battle_cursor.cursor_y[battle_master->battle_cursor.cursor_pos];
}
...
void update_cursor_move_select(u8 t_id)
{
    ...
    switch (gMain.newKeys & (KEY_DPAD | KEY_A | KEY_B)) {
        case KEY_B:
        {
            ...
            task_del(t_id);
            obj_free(&gSprites[battle_master->battle_cursor.objid_mv_crsr[0]]);
            obj_free(&gSprites[battle_master->battle_cursor.objid_mv_crsr[1]]);
            obj_free(&gSprites[battle_master->battle_cursor.objid]);
            gSprites[battle_master->battle_cursor.objid].final_oam.affine_mode = 2;
            for (u8 i = 0; i < 4; i++) {
                if (battle_master->type_objid[i] != 0x3F) {
                    gSprites[battle_master->type_objid[i]].final_oam.affine_mode = 2;
                    gSprites[battle_master->move_pss_objid[i]].final_oam.affine_mode = 2;
                    gSprites[battle_master->move_pp_objid[i]].final_oam.affine_mode = 2;
                }
                gSprites[battle_master->move_name_objid[i]].final_oam.affine_mode = 2;
            }
            bs_anim_status = 0;
            gMain.state = 0;
            return;
        ...
        case KEY_A:
            ...
            for (u8 i = 0; i < 4; i++) {
                if (battle_master->type_objid[i] != 0x3F) {
                    obj_free(&gSprites[battle_master->type_objid[i]]);
                    obj_free(&gSprites[battle_master->move_pss_objid[i]]);
                    obj_free(&gSprites[battle_master->move_pp_objid[i]]);
                }
                obj_free(&gSprites[battle_master->move_name_objid[i]]);
            }
            bs_anim_status = 0;
            task_del(t_id);
            gMain.state = 8;
    ...
    for (i = 0; i < 4; i++) {
        ...
        if (i == adjust) {
            gSprites[battle_master->type_objid[i]].final_oam.affine_mode = 2;
            gSprites[battle_master->move_pss_objid[i]].final_oam.affine_mode = 0;
            gSprites[battle_master->move_pp_objid[i]].final_oam.affine_mode = 0;
        } else {
            gSprites[battle_master->type_objid[i]].final_oam.affine_mode = 0;
            gSprites[battle_master->move_pss_objid[i]].final_oam.affine_mode = 2;
            gSprites[battle_master->move_pp_objid[i]].final_oam.affine_mode = 2;
        }
    }
    if (battle_master->battle_cursor.cursor_pos > 3) {
        gSprites[battle_master->battle_cursor.objid].final_oam.affine_mode = 0;
        gSprites[battle_master->battle_cursor.objid_mv_crsr[0]].final_oam.affine_mode = 2;
        gSprites[battle_master->battle_cursor.objid_mv_crsr[1]].final_oam.affine_mode = 2;
    } else {
        gSprites[battle_master->battle_cursor.objid].final_oam.affine_mode = 2;
        gSprites[battle_master->battle_cursor.objid_mv_crsr[0]].final_oam.affine_mode = 0;
        gSprites[battle_master->battle_cursor.objid_mv_crsr[1]].final_oam.affine_mode = 0;
        gSprites[battle_master->battle_cursor.objid_mv_crsr[0]].pos1.x = battle_master->battle_cursor.cursor_x[battle_master->battle_cursor.cursor_pos];
        gSprites[battle_master->battle_cursor.objid_mv_crsr[0]].pos1.y = battle_master->battle_cursor.cursor_y[battle_master->battle_cursor.cursor_pos];
        gSprites[battle_master->battle_cursor.objid_mv_crsr[1]].pos1.x = battle_master->battle_cursor.cursor_x[battle_master->battle_cursor.cursor_pos] + 64;
        gSprites[battle_master->battle_cursor.objid_mv_crsr[1]].pos1.y = battle_master->battle_cursor.cursor_y[battle_master->battle_cursor.cursor_pos];
    }
...
void spawn_square_pointer(u8 tag)
{
    ...
    gSprites[battle_master->battle_cursor.objid_mv_crsr[1]].final_oam.tile_num += 32;
}
...
void show_move_data(void)
{
    ...
    for (i = 0; i < 4; i++) {
        if (battle_master->type_objid[i] != 0x3F)
            gSprites[battle_master->type_objid[i]].final_oam.affine_mode = 0;
        gSprites[battle_master->move_name_objid[i]].final_oam.affine_mode = 0;
    }

    spawn_square_pointer(5);
    spawn_pointer_arrow(10);
    gSprites[battle_master->battle_cursor.objid].pos1.x = battle_master->battle_cursor.cursor_x[4];
    gSprites[battle_master->battle_cursor.objid].pos1.y = battle_master->battle_cursor.cursor_y[4];
    gSprites[battle_master->battle_cursor.objid].final_oam.affine_mode = 2; // hide
}
